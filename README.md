...
Bash command line tools to request Google Adwords API Reports with AWQL language and print results like mysql command line tool.
 
## Features

* Save results in CSV files.
* Caching datas in order to do not request Google Adwords services with queries already fetch in the day. This feature can be disable with live option (-l). 
* Add following SQL methods to AWQL grammar: `LIMIT` and `ORDER BY` in `SELECT` queries, `DESC` and `SHOW TABLES`.
...
```bash
./awql.sh -i "123-456-7890" -e "DESC CREATIVE_CONVERSION_REPORT;"
+--------------------------+---------------------------------------------------------------------------------------+------+
| Field                    | Type                                                                                  | Key  |
+--------------------------+---------------------------------------------------------------------------------------+------+
...
```bash
./awql.sh -i "123-456-7890" -e 'SHOW TABLES LIKE "CAMPAIGN%";'
+--------------------------------------------------+
...
```bash
./awql.sh -i "123-456-7890" -e "SHOW TABLES WITH 'ViewThroughConversions';"
+------------------------------------------+
| Tables_in_v201509ViewThroughConversions  |
+------------------------------------------+
...
```bash
./awql.sh -i "123-456-7890" -e "SELECT CampaignName, Clicks, Impressions, Cost, TrackingUrlTemplate FROM CAMPAIGN_PERFORMANCE_REPORT LIMIT 3"
+-----------+---------+--------------+------------+--------------------+
...
```bash
./awql.sh -i "123-456-7890" -e "SELECT CampaignName, Clicks, Impressions FROM CAMPAIGN_PERFORMANCE_REPORT ORDER BY Impressions DESC"
+--------------+---------+--------------+
...
Usage: ${SCRIPT} -i adwordsid [-a authfilepath] [-f awqlfilename] [-e query] [-l] [-v]
...
* -l used to disable cache
* -v used to print more informations

```bash
./awql.sh -i "123-456-7890" -e "SELECT CampaignName, Clicks, Impressions, Cost, TrackingUrlTemplate FROM CAMPAIGN_PERFORMANCE_REPORT LIMIT 5" -v    
+------------+---------+--------------+------------+--------------------+
| Campaign   | Clicks  | Impressions  | Cost       | Tracking template  |
+------------+---------+--------------+------------+--------------------+
| @0 #sp     | 12      | 1289         | 9760000    |                    |
...
| Mode #1    | 0       | 0            | 0          |                    |
+------------+---------+--------------+------------+--------------------+
5 rows in set @source /tmp/awql/20151206/3576955710_5.awql @cached
````